...
int32_t gluint_luna(tTensor *X, tTensor *Y, tTensor *workspace, GluIntAttrs *attr) {
    ...
    if (align_size <= 16384) {
        THINKER_RET_CHECK(API_LIB(mat_trans_i8o8)(input, input_trans, M, N), "luna_mat_trans_i8o8");
    } 
    else if (align_size <= 32768) {
        THINKER_RET_CHECK(API_LIB(split_mat_trans_i8o8)(input, input_trans, M, N), "luna_split_mat_trans_i8o8");
    } 
    else {
        return T_ERR_FAIL;
    }
    ...
    THINKER_RET_CHECK(API_LIB(scale_i8i8o16)(input_trans, 1, input_i16, size, 0), "luna_scale_i8i8o16");
    THINKER_RET_CHECK(API_LIB(scale_i16i16o32)(input_i16, 1, input_i32, size, 0), "luna_scale_i16i16o32");
    THINKER_RET_CHECK(API_LIB(scale_i32i32o32)(p_b, 1UL << shift, p_b, half_size, 0), "luna_scale_i32i32o32");
    THINKER_RET_CHECK(API_LIB(sigmoid_i32o32)(p_b, p_b, half_size), "luna_sigmoid_i32o32");
    THINKER_RET_CHECK(API_LIB(scale_i32i32o32)(p_b, 1, p_b, half_size, 16), "luna_scale_i32i32o32"); // Convert to Q15
    shift = Q_SIGMOID_OU + X->scale_ - Y->scale_;
    THINKER_RET_CHECK(API_LIB(mul_i32i32o8)(p_a, p_b, output_temp, half_size, shift), "luna_mul_i32i32o8");

    // Final matrix transpose for output
    if (align_size <= 32768) {
        THINKER_RET_CHECK(API_LIB(mat_trans_i8o8)(output_temp, output, N / 2, M), "luna_mat_trans_i8o8");
    } 
    else if (align_size <= 65536) {
        THINKER_RET_CHECK(API_LIB(split_mat_trans_i8o8)(output_temp, output, N / 2, M), "luna_split_mat_trans_i8o8");
    } 
    else {
        return T_ERR_FAIL;
    }
